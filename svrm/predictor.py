...
class MV23DPredictor(object):
    def __init__(
        ...
        self.final_input_view_transform = transforms.Normalize(
    ...
    def refine_mask(self, mask, k=16):
        mask /= 255.0
        boder_mask = (mask >= -math.pi / 2.0 / k + 0.5) & (
            mask <= math.pi / 2.0 / k + 0.5
        )
    ...
    def load_images_and_cameras(self, input_imgs, elevation_list, azimuth_list):
        input_image_list = []
        input_cam_list = []
        for input_view_image, elevation, azimuth in zip(
            input_imgs, elevation_list, azimuth_list
        ):
            input_view_image = self.input_view_transform(input_view_image)
            input_image_list.append(input_view_image)

            input_view_cam_pos = self.create_camera_to_world_matrix(
                np.radians(elevation), np.radians(azimuth)
            )
            input_view_cam_intrinsic = np.array([35.0 / 32, 35.0 / 32, 0.5, 0.5])
            input_view_cam = torch.from_numpy(
                np.concatenate(
                    [input_view_cam_pos.reshape(-1), input_view_cam_intrinsic], 0
                )
            ).float()
    ...
    def load_data(self, intput_imgs):
        ...
        input_images, input_cams = self.load_images_and_cameras(
            intput_imgs, self.elevation_list, self.azimuth_list
        )
    ...
    def predict(
        ...
        with torch.cuda.amp.autocast():
            return self.model.export_mesh_with_uv(
